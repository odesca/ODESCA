...
classdef Test_ODESCA_System_ComponentBroken < ODESCA_Component
    %ODESCA_SystemTest_Component2 Class for the test of ODESCA_System
    %
    % DESCRIPTION
    %   This class is used for the tests for the ODESCA_System class.
    %   It has the construction parameter c and broken equations.
    %
    % PROPERTIES:
    %
    % CONSTRUCTOR:
    %   obj = ODESCA_SystemTest_Component2()
    %
    % METHODS:
    %
    % LISTENERS:
    ...
    methods
        function obj = Test_ODESCA_System_ComponentBroken(name)
    ...
    methods(Access = protected)
        function calculateEquations(obj)
            ...
            obj.f(1) = - State + Input * Parameter;
            obj.f(2) = - 1 + State;
            obj.g(1) = (State + Input + Parameter + sym('k'))^(obj.constructionParam.c);
