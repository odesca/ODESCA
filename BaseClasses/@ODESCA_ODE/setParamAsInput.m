...
if( ~isa(paramName,'char') || size(paramName,1) ~= 1 )
    error('ODESCA_ODE:setParamAsInput:ParameterNameMustBeString','Argument ''paramName'' must be of type string.');
end

% Check if the ODE has parameters
if( isempty(obj.param) )
    error('ODESCA_ODE:setParamAsInput:NoParametersExist','The ODE has no parameters which could be set as input.');
end
...
if( ~ismember(paramName, existingParam) )
    error('ODESCA_ODE:setParamAsInput:NotAParameter',['The parameter ''',paramName,''' does not exist.']);
end

% Check if an input with the same name already exists
if( ismember(paramName, obj.inputNames))
    error('ODESCA_ODE:setParamAsInput:isAlreadyInput','An input with the same name as the parameter already exists.');
end
