...
if( isempty(obj.param) )
    error('ODESCA_ODE:renameParam:noParametersInODE','The ODE has no parameters.');
end

% Check if oldName is a string
if( ~ischar(oldName) || size(oldName,1) ~= 1)
    error('ODESCA_ODE:renameParam:oldNameNotAString','The old name has to be a string.');
end
...
if( ~ismember(oldName ,paramNames) )
    error('ODESCA_ODE:renameParam:oldNameNotInODE',['The ODE has no parameter with the name ''',oldName,'''.']);
end

% Check if the new name is valid
if( ~isvarname(newName))
    error('ODESCA_ODE:renameParam:newNameNotValid','The Argument ''newName'' has to match the naming conventions of MATLAB variables.');
end
...
if( ismember(newName ,paramNames) || ismember(newName ,obj.inputNames) || ismember(newName ,obj.stateNames) )
    error('ODESCA_System:renameParam:newNameAlreadyInODE',['The ODE already has a parameter, state or input with the name ''',newName,'''.']);
end
