...
if( isa(state1, 'char') && size(state1, 1) == 1 && ...
    ...
    if( pos1 == -1 )
        error('ODESCA_ODE:switchStates:InputNotFound',['There is no state with the name ''',state1,'''.']);
    end
    if( pos2 == -1 )
        error('ODESCA_ODE:switchStates:InputNotFound',['There is no state with the name ''',state2,'''.']);
    end
...
elseif( isnumeric(state1) && isnumeric(state2) && numel(state1)==1 && numel(state2)==1 )
    ...
    if( state1 <= 0 || mod(state1,1) ~= 0 || state1 > l_in || ...
        ...
        error('ODESCA_ODE:switchStates:InvalidArryIndex','The indexs of the states must be integer values between 1 and the number of states.');
    end
    ...
    error('ODESCA_ODE:switchStates:InvalidInputArguments','The input arguments must be either both of type string or both of type integer.');
end
