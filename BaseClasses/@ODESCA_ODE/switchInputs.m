...
if( isa(in1, 'char') && size(in1, 1) == 1 && ...
    ...
    if( pos1 == -1 )
        error('ODESCA_ODE:switchInputs:InputNotFound',['There is no intput with the name ''',in1,'''.']);
    end
    if( pos2 == -1 )
        error('ODESCA_ODE:switchInputs:InputNotFound',['There is no intput with the name ''',in2,'''.']);
    end
...
elseif( isnumeric(in1) && isnumeric(in2) && numel(in1)==1 && numel(in2)==1 )
    ...
    if( in1 <= 0 || mod(in1,1) ~= 0 || in1 > l_in || ...
        ...
        error('ODESCA_ODE:switchInputs:InvalidArryIndex','The indexs of the inputs must be integer values between 1 and the number of inputs.');
    end
    ...
    error('ODESCA_ODE:switchInputs:InvalidInputArguments','The input arguments must be either both of type string or both of type integer.');
end
