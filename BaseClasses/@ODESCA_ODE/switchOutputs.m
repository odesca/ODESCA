...
if( isa(out1, 'char') && size(out1, 1) == 1 && ...
    ...
    if( pos1 == -1 )
        error('ODESCA_ODE:switchOutputs:OutputNotFound',['There is no output with the name ''',out1,'''.']);
    end
    if( pos2 == -1 )
        error('ODESCA_ODE:switchOutputs:OutputNotFound',['There is no output with the name ''',out2,'''.']);
    end 
...
elseif( isnumeric(out1) && isnumeric(out2) && numel(out1)==1 && numel(out2)==1  )
    ...
    if( out1 <= 0 || mod(out1,1) ~= 0 || out1 > l_out || ...
        ...
        error('ODESCA_ODE:switchOutputs:InvalidArryIndex','The indexs of the outputs must be integer values between 1 and the number of outputs.');
    end
    ...
    error('ODESCA_ODE:switchOutputs:InvalidInputArguments','The input arguments must be either both of type string or both of type integer.');
end
