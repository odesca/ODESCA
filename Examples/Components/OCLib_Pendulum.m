...
classdef OCLib_Pendulum < ODESCA_Component
    %
    % DESCRIPTION
    %   A model of a pendulum
    %
    % PROPERTIES:
    %
    % CONSTRUCTOR:
    ...
    %
    % NOTE:
    %   The angle being zero means the pendulum is in its unstable 
    %   equilibrium point (vividly twelve o'clock').
    %
    % SEE ALSO
    %   PS600 by Amira
    %
    ...
    methods
        function obj = OCLib_Pendulum(name)
    ...
    methods(Access = protected)
        function calculateEquations(obj)
            ...
            stateNames  = {'Position', 'Velocity', 'Angle', 'AngularSpeed'};
            stateUnits  = {'m'       , 'm/s'     , 'rad'  , 'rad/s'       };
            
            inputNames  = {'Force'};
            inputUnits  = {'N'    };
            
            outputNames = {'Position', 'Angle'};
            outputUnits = {'m'       , 'rad'  };
            
            paramNames  = {'M0', 'M1', 'l_s', 'theta', 'Fr', 'C', 'g'};
            paramUnits  = {'kg', 'kg', 'm', 'kgm^2', 'kg/s', 'kgm^2/s', 'm/s^2'};
            % M0: mass of trolley
            % M1: mass of pendulum
            % l_s: length of pendulum (center of mass)
            % theta: inertia moment
            % Fr: friction of trolley
            % C: friction of pendulum
            ...
            N = M1*l_s;
            N01 = theta*M-N^2;
            beta(Angle) = (1+N^2/N01*sin(Angle)^2)^-1;
            ...
            obj.f(1) = obj.x(2);
            obj.f(3) = obj.x(4);
            obj.f(2) = beta(Angle)*(a23*sin(Angle)*cos(Angle)+a22*Velocity+a24*cos(Angle)*AngularSpeed+a25*sin(Angle)*AngularSpeed^2+b2*Force);
            obj.f(4) = beta(Angle)*(a43*sin(Angle)+a42*cos(Angle)*Velocity+a44*AngularSpeed+a45*cos(Angle)*sin(Angle)*AngularSpeed^2+b4*cos(Angle)*Force);
            obj.g(1) = obj.x(1);
            obj.g(2) = obj.x(3);
